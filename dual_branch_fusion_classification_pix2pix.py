...
def expand_weight_in_state_dict(state_dict: dict, key: str, in_ch_new: int):
    ...
    if in_ch_new < in_c_old:
        # 例如 DINO 预训练权重 3 通道迁移到 SAR 1 通道时做均值压缩
...
class DualBranchHybridFusionNet(nn.Module):
    def __init__(
        self,
        num_classes: int = 6,
        sar_arch: str = "resnet18",
        opt_arch: str = "resnet18",
        sar_ckpt: str = None,
        opt_ckpt: str = None,
        fusion_dim: int = 512,
    ):
...
def main():
    ...
    parser.add_argument("--backbone_arch", type=str, default="dino",
                        choices=["resnet18", "resnet34", "resnet50", "dino", "dinov2"])
    parser.add_argument("--sar_arch", type=str, default="dino",
                        choices=["resnet18", "resnet34", "resnet50", "dino", "dinov2"])
    parser.add_argument("--opt_arch", type=str, default="dino",
                        choices=["resnet18", "resnet34", "resnet50", "dino", "dinov2"])
    ...
    best_acc = 0.0
    best_path = "classification_checkpoints/best_dual_branch_fusion_pix2pix_dino.pth"
    ...
    for ep in range(1, args.epochs + 1):
        ...
        compute_and_save_metrics(
            ...
            save_dir="metrics_dual_branch_pix2pix_dino",
