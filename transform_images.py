...
class EdgesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        transform_imageA = get_transform(params1, self.resize_size, self.crop_size, crop=self.random_crop,
                                        flip=self.random_flip)
        transform_imageB = get_transform(params2, self.resize_size, self.crop_size, crop=self.random_crop,
                                        flip=self.random_flip)
        A = transform_imageA(A)
        B = transform_imageB(B)
...
#     root = "/data/yjy_data/Parallel-GAN-main/Parallel-GAN-main/datasets/sar2opt/combine"
#     trainset = EdgesDataset(dataroot=root, train=True, img_size= 600,
#                                 random_crop=True, random_flip=False)
#     idx = 0
#     B_crop, A_crop, _ = trainset[idx]  # 经过crop/transform的版本
#
#     # 读取原始拼接图并分割
#     AB_path = trainset.AB_paths[idx]
#     AB_img = Image.open(AB_path).convert("RGB")
#     w, h = AB_img.size
#     w2 = w // 2
#
#     A_raw = AB_img.crop((0, 0, w2, h))
#     B_raw = AB_img.crop((w2, 0, w, h))
#
#     #
#     # # ========================
#     # # 反归一化函数（若数据范围为 [-1,1]）
...
#     # 创建保存目录结构
#     # ========================
#     save_root = "/data/yjy_data/DDBM_GT_Unet/crop_result/sample_{:03d}".format(idx)
#     os.makedirs(os.path.join(save_root, "raw"), exist_ok=True)
#     os.makedirs(os.path.join(save_root, "cropped"), exist_ok=True)
#
#     # ========================
#     # 保存原始未crop图像
#     # ========================
#     B_raw.save(os.path.join(save_root, "raw", "B_raw.png"))
#     A_raw.save(os.path.join(save_root, "raw", "A_raw.png"))
#
#     # ========================
#     # 保存经过crop后的图像
#     # ========================
#     save_image((B_crop), os.path.join(save_root, "cropped", "B_crop.png"))
#     save_image((A_crop), os.path.join(save_root, "cropped", "A_crop.png"))
#     # save_image(denorm(B_crop), os.path.join(save_root, "cropped", "B_crop.png"))
#     # save_image(denorm(A_crop), os.path.join(save_root, "cropped", "A_crop.png"))
#
#     print(f"✅ 样本 {idx} 的四张图已保存到 {save_root}/")
#     print("├── raw/B_raw.png")
#     print("├── raw/A_raw.png")
#     print("├── cropped/B_crop.png")
#     print("└── cropped/A_crop.png")
