...
class SAROptDataset(Dataset):
    def __init__(self, sar_dir, opt_dir, transform=None):
        self.sar_files = sorted(glob.glob(os.path.join(sar_dir, "*.jpg")))
        self.opt_files = sorted(glob.glob(os.path.join(opt_dir, "*.jpg")))
...
# ============================================================
def gpu_edge_approx(images, for_optical=False, strength=2.0):
    """
    自适应边缘检测,光学图像会自动增强

    Args:
        images: [B,C,H,W]
        for_optical: 是否为光学图像(会应用更强的增强)
        strength: 增强强度,越大边缘越明显
    """
    # 1. 预处理:对光学图像做对比度增强
    if for_optical:
        # CLAHE式局部对比度增强
        mean_val = images.mean(dim=[2, 3], keepdim=True)
        std_val = images.std(dim=[2, 3], keepdim=True)
        images = (images - mean_val) / (std_val + 1e-8)
    ...
    edges = edges.mean(dim=1, keepdim=True)  # 合并通道

    # 4. 自适应增强
    if for_optical:
        # 非线性增强(Gamma校正)
        edges = torch.pow(edges + 1e-8, 1.0 / strength)  # strength=2.0时相当于开平方

        # 自适应阈值化(保留强边缘)
        threshold = edges.mean() + 0.5 * edges.std()
        edges = torch.where(edges > threshold, edges * 1.5, edges * 0.3)
...
if __name__ == "__main__":
    test_model(
        model_path="/data/yjy_data/DDBM_GT_Unet/canny_optimization_result_SAR2OPT_v2/unet_epoch_120.pth",
        sar_dir="/data/yjy_data/Parallel-GAN-main/Parallel-GAN-main/datasets/sar2opt/testA/",
        opt_dir="/data/yjy_data/Parallel-GAN-main/Parallel-GAN-main/datasets/sar2opt/testB",
        save_dir="/data/yjy_data/DDBM_GT_Unet/canny_optimization_result_SAR2OPT_v2/test_result_120_all/",
        batch_size=4
    )
