...
import einops
from ddbm.fp16_util import convert_module_to_f16, convert_module_to_f32
from .nn import (
    checkpoint,
    conv_nd,
    linear,
...
from .GT_Unet import BotBlock,_make_bot_layer
from .SEBlock import FeatureFusionNet

# 定义SE模块
class SEBlock(th.nn.Module):
    def __init__(self, in_channels, reduction=16):
        super(SEBlock, self).__init__()
        self.fc1 = th.nn.Linear(in_channels, in_channels // reduction)
        self.fc2 = th.nn.Linear(in_channels // reduction, in_channels)
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x, encoder_out=None):
        ...
        if b == 1:  # 仅保存 batch size = 1 的情况，方便调试
            # 选择某个 head（例如第一个）
            q_image = q[0, 0]  # [H * W]
            q_image = q_image.view(int(spatial[0]), int(spatial[1]))  # 变成 2D 图像

            # 将 tensor 转成 numpy 格式以便保存
            img = tensor_to_image(q_image)
            plt.imshow(img, cmap='viridis')
            plt.colorbar()
            plt.title("Query Feature Map")
            plt.axis('off')
            plt.savefig('query_feature_map.png')  # 保存图片
            plt.close()
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.Bot1 = _make_bot_layer(ch_in=ch, ch_out=64)
                ...
                if block_idx == 1:
                    layers.append(
                        BotBlock(
                            ...
                            curr_h=4,
                            curr_w=4,
                            # target_dimension=int(mult * model_channels),
                            target_dimension=int(mult * model_channels),
                            stride=1
                        )
                    )
        ...
        self.se_block = FeatureFusionNet(use_fp16=True)
    ...
    def convert_to_fp16(self):
        ...
        self.se_block.apply(convert_module_to_f16)
