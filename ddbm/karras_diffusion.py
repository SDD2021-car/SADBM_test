...
from SAB.ConvNetworkWithImageFeature import canny_edge_detection,apply_canny_to_batch
from .nn import mean_flat, append_dims, append_zero
from SAB.ConvNetworkWithImageFeature import apply_canny_to_batch
...
class KarrasDenoiser:
    ...
    def get_bridge_scalings(self, sigma):
        ...
        if self.pred_mode == 've':
            A = sigma ** 4 / self.sigma_max ** 4 * self.sigma_data_end ** 2 + (
                        1 - sigma ** 2 / self.sigma_max ** 2) ** 2 * self.sigma_data ** 2 + 2 * sigma ** 2 / self.sigma_max ** 2 * (
                            1 - sigma ** 2 / self.sigma_max ** 2) * self.cov_xy + sigma ** 2 * (
                            1 - sigma ** 2 / self.sigma_max ** 2)
            c_in = 1 / (A) ** 0.5
            ...
            c_out = ((sigma / self.sigma_max) ** 4 * (self.sigma_data_end ** 2 * self.sigma_data ** 2 - self.cov_xy ** 2) +
                     self.sigma_data ** 2 * sigma ** 2 * (1 - sigma ** 2 / self.sigma_max ** 2)) ** 0.5 * c_in
    ...
    def training_bridge_losses(self, model, x_start, sigmas, model_kwargs=None, noise=None, vae=None,self_param = None):
        ...
        # 建立一个和x_start相同尺度的噪声，范围在 sigma_min到sigma_max间随机选择
        B = x_start.shape[0]
        device = x_start.device
        sigmas = th.rand(B, device=device) * (self.sigma_max - self.sigma_min) + self.sigma_min
        # sigmas = sigmas.to("cuda:1")
        terms = {}
        dims = x_start.ndim
        ...
        # denoised_canny= apply_canny_to_batch(denoised).to("cuda:7")
        # x_start_canny=apply_canny_to_batch(x_start).to("cuda:7")
        lambda_freq = 0.01
        # 计算高频信息的MSE
        # mse_high_freq = mean_flat(weights * (denoised_canny - x_start_canny) ** 2)
        terms["xs_mse"] = mean_flat((denoised - x_start) ** 2)
        # terms["mse"] = mean_flat(weights * (denoised - x_start) ** 2)+lambda_freq * mse_high_freq

        if self_param.stage0_train:
            terms["mse"] = mean_flat(weights * (denoised - x_start) ** 2 ) + nn.MSELoss(self_param.canny_SAR,apply_canny_to_batch(x_start))
        else:
    ...
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        ...
        return model_output, denoised  # D
