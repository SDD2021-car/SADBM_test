...
def main():
    ...
    parser.add_argument("ref_batch", default='/data/yjy_data/DDBM/edges2handbags_ref_64_data.npz',help="path to reference batch npz file")
    parser.add_argument("sample_batch", default='/data/yjy_data/DDBM/logs3/sample_90000/w=0.5_churn=0.33/samples_200x64x64x3_nfe299.npz',help="path to sample batch npz file")
...
def get_ssim_lpips(args):
    ...
    ds = torch.utils.data.TensorDataset(torch.from_numpy(ref_batch).permute(0,3,1,2), torch.from_numpy(sample_batch).permute(0,3,1,2))
...
class Evaluator:
    ...
    def read_statistics(
        self, npz_path: str, activations: Tuple[np.ndarray, np.ndarray], store=False,
    ...
    def compute_inception_score(
        self, logits: np.ndarray, split_size: int = 5000
    ) -> float:
        softmax_out = []
        for i in range(0, len(logits), self.softmax_batch_size):
            logits_batch = logits[i : i + self.softmax_batch_size]
            softmax_out.append(
                torch.nn.functional.softmax(torch.from_numpy(logits_batch), dim=-1).numpy()
            )
