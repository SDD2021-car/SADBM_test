...
# ============================================================
# 3️⃣ U-Net 模型定义（添加 Dropout）
# ============================================================
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, dropout_p=0.3):
        super().__init__()
        self.enc1 = self.block(in_channels, 64, dropout_p)
        self.enc2 = self.block(64, 128, dropout_p)
        self.enc3 = self.block(128, 256, dropout_p)
        self.enc4 = self.block(256, 512, dropout_p)
        self.bottleneck = self.block(512, 1024, dropout_p)
    ...
    def forward(self, x):
        ...
        d2 = self.up2(d3) + e2
        d1 = self.up1(d2) + e1
        out = self.final(d1)  # ⚠️ 去掉 sigmoid，交叉熵会自动处理
        return out
...
model = UNet(in_channels=1, out_channels=1, dropout_p=0.1).to(device)
...
num_epochs = 200
save_dir = "/data/yjy_data/DDBM_GT_Unet/canny_optimization_result_CE"
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        with torch.no_grad():
            ...
            sar_edge = gpu_edge_approx(sar)
            opt_edge = gpu_edge_approx(opt)
            pred = torch.sigmoid(model(sar_edge))  # ✅ 可视化时再加 sigmoid
